...
static void rejectChoice(bool confirm) {
  if (confirm){
    ...
    {
      ui_reviewStart(confirm_clbk,reject_clbk);
    }
  }
}
...
static void rejectUseCaseChoice(void)
{
    nbgl_useCaseChoice(NULL,"Reject transaction ?",NULL,"Yes, cancel","Go back to transaction",rejectChoice);
}
...
static void displayPath(void) {
    memset(&infoLongPress, 0, sizeof(infoLongPress));
    infoLongPress.text = "Confirm key export ?";
    infoLongPress.longPressText = "Confirm";
    infoLongPress.icon = &C_fantom_logo_64px;
    nbgl_useCaseStaticReviewLight(&pairList,&infoLongPress,"Cancel",&displayPathCallback);
}
...
void ui_reviewDisplay(ui_tx_fields_t *tx_fields, ui_callback_fn_t *confirm, ui_callback_fn_t *reject, bool contract_warning)
{
    ...
    if(contract_warning)
    {
        nbgl_useCaseChoice(&C_round_warning_64px,"Contract call","Review carefully","Continue","Reject",contractWarningChoice);
    }
    else
    {
        displayTransaction();
...
void ui_displayWarning(const char * warning_text, ui_callback_fn_t *confirm, ui_callback_fn_t *reject)
{
    ...
    confirm_clbk = confirm;
    reject_clbk = reject;
    nbgl_useCaseChoice(&C_round_warning_64px,(char*)warning_text,"Reject if you're not sure","Continue","Reject",warningChoice);
}
...
void ui_reviewStart(ui_callback_fn_t *confirm, ui_callback_fn_t *reject)
{
    ...
    nbgl_useCaseReviewStart(&C_fantom_logo_64px, "Review\ntransaction", NULL,
                            "Reject", confirm, rejectUseCaseChoice);
}
...
void ui_verifyAddress(ui_tx_fields_t *tx_fields, ui_callback_fn_t *confirm, ui_callback_fn_t *reject, bool show_addr,bool show_path){
    ...
    nbgl_useCaseReviewStart(&C_fantom_logo_64px,"Verify \naddress",NULL,"Cancel",
                          displayAddress, rejectAndDisplayIdleMenu);
}
...
void ui_exportKeyConfirm(ui_tx_fields_t *tx_fields, ui_callback_fn_t *confirm, ui_callback_fn_t *reject){
    ...
    nbgl_useCaseReviewStart(&C_fantom_logo_64px,"Export \npublic key",NULL,"Cancel",
                          displayPath, rejectAndDisplayIdleMenu);
}
#endif //  HAVE_NBGL
